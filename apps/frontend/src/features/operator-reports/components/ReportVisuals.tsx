...
export function ReportVisuals({ report }: { report: OperationsReport }) {
  return <section aria-label="Biểu đồ vận hành" className="grid gap-4 xl:grid-cols-2">
    <Card className="nf-workspace-panel xl:col-span-2">
      <ChartHeading description="Chỉ xếp hạng campaign có phát sinh hoạt động trong DB." title="Hiệu quả theo campaign" />
      <ReportCampaignChart campaigns={report.campaigns} />
    </Card>
    <Card className="nf-workspace-panel">
      <ChartHeading description="Theo dõi tiền từ giữ chỗ đến thanh toán hoặc hoàn trả." title="Dòng ngân sách" />
      <ReportBudgetChart summary={report.summary} />
    </Card>
    <Card className="nf-workspace-panel">
      <ChartHeading description="Nhìn nhanh tỷ trọng campaign theo trạng thái hiện tại." title="Trạng thái campaign" />
...
function ChartHeading({ description, title }: { description: string; title: string }) {
  return <div className="mb-2"><h2 className="font-semibold text-slate-950">{title}</h2><p className="mt-1 text-xs text-slate-500">{description}</p></div>
}
