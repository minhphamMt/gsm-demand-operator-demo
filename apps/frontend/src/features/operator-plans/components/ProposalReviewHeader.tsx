...
type Props = { actions: ReactNode; hasCampaign: boolean; plan: Proposal }

export function ProposalReviewHeader({ actions, hasCampaign, plan }: Props) {
  ...
  return (
    <Card className="p-0">
      <div className="flex flex-col justify-between gap-4 px-5 py-4 lg:flex-row lg:items-start">
        ...
        <div className="lg:text-right">{actions}<p className="mt-2 max-w-md text-xs text-slate-500">Phê duyệt khóa proposal. Việc phát hành offer luôn cần xác nhận riêng.</p></div>
      </div>
      <ol aria-label="Tiến trình proposal" className="grid border-y border-slate-200 bg-slate-50/70 text-xs sm:grid-cols-4">
        <Step done label="Gợi ý được ghi nhận" number="1" />
        <Step done label={plan.simulationAvailable ? 'Mô phỏng & policy' : 'Policy DB'} number="2" />
        <Step done={plan.status === 'Approved' || plan.status === 'Rejected'} current={plan.status === 'UnderReview' || plan.status === 'Revised'} label="Điều phối kiểm duyệt" number="3" />
        <Step done={hasCampaign} current={plan.status === 'Approved' && !hasCampaign} label="Kích hoạt offer" number="4" />
      </ol>
      <dl className="grid sm:grid-cols-2 xl:grid-cols-4">
        <Summary label="Vùng cần bổ sung" value={plan.targetZoneId ? plan.targetZoneLabel : 'Chưa xác định'} />
        <Summary label="Mục tiêu" value={`${plan.targetDriverCount} tài xế / ${plan.campaignDurationMinutes} phút`} />
        <Summary label="Độ tin cậy" value={formatOptionalPercentRatio(plan.confidence)} />
        <Summary label="Policy / ngân sách" value={`${passedPolicies}/${plan.policyChecks.length} đạt · ${formatCurrency(plan.budgetLimit)}`} />
      </dl>
    </Card>
  )
}
