...
export function PlanDetail({ activationRoute = routes.operator.campaigns, detailRoute = routes.operator.planDetail, planId }: { activationRoute?: string; detailRoute?: (planId: string) => string; planId: string }) {
  ...
  const decisionError = actionErrorMessage([actions.approve.error, actions.reject.error, actions.activate.error, actions.cancelCampaign.error])
  ...
  return <div className="nf-plan-detail-stack space-y-5">
    ...
    <ProposalReviewHeader
      plan={proposal}
      hasCampaign={Boolean(linkedCampaign)}
      actions={decisionActions}
      campaignAction={hasOperationalCampaign && linkedCampaign ? <CampaignOfferAction campaignId={linkedCampaign.id} isWorking={actions.cancelCampaign.isPending} onCancel={(campaignId) => actions.cancelCampaign.mutate(campaignId)} /> : undefined}
    />
...
function CampaignOfferAction({ campaignId, isWorking, onCancel }: { campaignId: string; isWorking: boolean; onCancel: (campaignId: string) => void }) {
  const [isOpen, setIsOpen] = useState(false)
  return <>
    <Button variant="danger" onClick={() => setIsOpen(true)}>Hủy offer</Button>
    <Dialog isOpen={isOpen} onClose={() => setIsOpen(false)} title="Hủy offer đang phát hành">
      <p className="text-sm text-slate-600">Các offer đang chờ sẽ được hủy và tài xế được giải phóng. Offer đã nhận vẫn được giữ để đối soát.</p>
      <div className="mt-5 flex justify-end gap-2"><Button variant="secondary" onClick={() => setIsOpen(false)}>Quay lại</Button><Button variant="danger" isLoading={isWorking} onClick={() => { onCancel(campaignId); setIsOpen(false) }}>Xác nhận hủy</Button></div>
    </Dialog>
  </>
}
