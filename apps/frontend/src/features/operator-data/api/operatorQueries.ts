...
export const operatorQueryKeys = { capabilities: ['operator', 'capabilities'] as const, policy: ['operator', 'policy'] as const, dispatch: ['operator', 'dispatch'] as const, notifications: ['operator', 'notifications'] as const, snapshot: (comparison: Scenario, scenario: DemoScenarioId, replay: number) => ['operator', 'snapshot', comparison, scenario, replay] as const, replaySnapshot: (sourceAt: string) => ['operator', 'replay-snapshot', sourceAt] as const, baselines: ['operator', 'baselines'] as const, plans: ['operator', 'plans'] as const, plan: (id: string) => ['operator', 'plans', id] as const, campaigns: ['operator', 'campaigns'] as const, report: (filters: OperationsReportFilters) => ['operator', 'reports', 'operations', filters] as const, offerRoot: ['operator', 'offers'] as const, offers: (campaignId?: string) => ['operator', 'offers', campaignId ?? 'all'] as const, audit: ['operator', 'audit'] as const, auditPage: (filters: AuditFilters) => ['operator', 'audit', 'page', filters] as const, driverRoot: ['driver'] as const, drivers: ['driver', 'list'] as const, driver: (id: string) => ['driver', id] as const }
// I-08 khoá `policy.yaml`, nên ngưỡng không đổi giữa chừng một phiên làm việc. Không poll:
// bảng chỉ số nhấp nháy trong lúc người dùng đang kéo thanh trượt là cách nhanh nhất để họ
// chỉnh nhầm một số. `retry: false` vì AI service chết thì bảng phải nói ra, không treo.
export const policyQuery = () => queryOptions({ queryKey: operatorQueryKeys.policy, queryFn: operatorAdapter.getPolicy, staleTime: Infinity, retry: false })
