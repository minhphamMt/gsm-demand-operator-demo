...
export function AuditFilterForm({ filters, onApply, onReset }: { filters: AuditUrlFilters; onApply: (filters: AuditUrlFilters) => void; onReset: () => void }) {
  ...
  return <Card className="nf-workspace-panel nf-history-filter-card p-4"><form onSubmit={(event) => { event.preventDefault(); if (!entityError && !dateError) onApply({ ...draft, page: 1 }) }}>
