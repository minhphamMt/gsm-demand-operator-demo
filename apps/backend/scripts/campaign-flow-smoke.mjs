...
async function cleanupFixtures(driverId, previousDriverState) {
  const { campaignIds, entityIds } = await fixtureEntityIds();
  ...
  await adminDb.from('audit_logs').delete().in('entity_id', entityIds);
  await adminDb.from('audit_logs').delete().eq('entity_id', driverId).eq('action', 'DriverStatusChanged').eq('metadata->>request_id', 'audit-smoke-driver-status');
...
try {
  ...
  await api(baseUrl, `/api/v1/drivers/${driver.userId}/status`, driver.token, {
    ...
    requestId: 'audit-smoke-driver-status',
  });
  const { data: driverStatusAudit } = await adminDb.from('audit_logs')
    ...
    .eq('metadata->>request_id', 'audit-smoke-driver-status');
  ...
  if (releasedParticipation?.status !== 'CANCELLED' || releasedDriver?.active_campaign_id) throw new Error('Cancellation did not release accepted participation and driver');
  ...
  const { error: completedReconcileError } = await adminDb.rpc('reconcile_campaign_lifecycle', { p_request_id: 'campaign-smoke-completed' });
  ...
  if (completedRow?.status !== 'COMPLETED' || completedOffers?.some(({ status }) => status !== 'EXPIRED')
    ...
    || completedAudit[0].metadata?.request_id !== 'campaign-smoke-completed') {
  ...
  if (targetAudit?.length !== 1 || targetAudit[0].metadata?.request_id !== 'audit-smoke-offer-accept-target'
    || !targetAudit[0].actor_id || !targetAudit[0].before_data || !targetAudit[0].after_data
    || offerAudit?.length !== 1 || offerAudit[0].metadata?.request_id !== 'audit-smoke-offer-accept-target'
