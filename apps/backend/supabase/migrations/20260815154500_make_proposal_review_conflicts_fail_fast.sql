...
-- A duplicate click/retry must not occupy a connection while another command
-- with the same key is still completing. Fail quickly and let the client retry
-- the same idempotency key, whose final response will be replayed by this RPC.
...
begin
  ...
  exception
    ...
    when lock_not_available then
      raise exception using errcode = '40001', message = 'Proposal review is already being processed';
  end;
