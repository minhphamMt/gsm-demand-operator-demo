...
def build_snapshot(zones, gen_cfg, policy, split="train"):
    ...
    rain_csv = f"{gen_cfg.get('_base_dir', '.')}/data/rain_hanoi_2025.csv"
    if split == "train":
    ...
    for zone in zones:
        ...
        for i, ts in enumerate(timestamps):
            ...
            # price_index (SURGE MULTIPLIER) - cong thuc dong, neo vao gap demand/supply
            # gia_index = 1.0 + min(1.0, gap/demand) * 0.5, chan trong [1.0, 2.0]
            # 0.5 la he so tuong duong muc phu thu gio cao diem thi truong that (~15-20%),
            # nhan len khi thieu cung nghiem trong (gap/demand cao) - ASSUMPTION, chua duoc PM duyet
            gap = demand - supply
            gap_ratio = max(0, gap / demand) if demand > 0 else 0
            price_index = round(1.0 + min(1.0, gap_ratio) * 0.5, 3)
            ...
            rows.append({
                ...
                "price_index": price_index,  # surge dong, xem pricing_policy_draft.yaml - VAN khong dung lam feature Model 1/2
            })
