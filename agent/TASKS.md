...
>
> *(Cập nhật 28/08/2026: quyết định #9 "theme giữ light" đã bị đảo ở MA-4.26 — bảng điều hành
> nay dùng nền tối, khai báo cục bộ trên `.nf-ops`, không đụng `@theme` toàn cục.)*

---

## Phase 6 — AI Agent Interaction Log ⬜

**Kế hoạch chi tiết: [`08-interaction-log-plan.md`](08-interaction-log-plan.md).** Neo tài liệu:
...
| MA-6.7 | Nâng `usePipelineRun` lên dashboard + popup `AgentInteractionLog` | ✅ | MA-6.5 | `AgentInteractionLog.tsx` + hook nâng lên dashboard. Popup **chỉ có một nút**, và nó chỉ đổi cách nhìn — test `không mang bất kỳ nút hành động nào` khẳng định đúng điều đó. Chạy thật bằng Playwright: 8 → 12 → 20 → 23 dòng qua bốn lát cắt. Bốn chỗ lệch so với kế hoạch ghi ở `08` §4 Chặng 2 |
| MA-6.8 | Thao tác người vận hành vào cùng log | ✅ | MA-6.7 | Ghi ở 10 hành động tại `onSuccess`/`onError`, **không** lúc bấm — bấm là ý định, không phải sự việc. **Không dùng Context** như kế hoạch dự kiến: cả mười handler nằm trong chính `OperatorConsoleDashboard` nên không có ranh giới nào để vượt (xem `08` §4 Chặng 3). Test tích hợp bấm thật: `[NGƯỜI VẬN HÀNH] > đã chạy dự báo horizon 15 phút`; mutation check → đỏ |
| MA-6.9 | Bước thực thi sau duyệt + system prompt + narration LLM | ✅ | MA-6.8 | Ba chủ thể chứ không một: `[THỰC THI]` (backend) · `[TÀI XẾ]` (nhận/từ chối offer — quyền của họ theo C-08, không phải bước thực thi) · `[NGƯỜI VẬN HÀNH]`. Trùng dòng chấp nhận, bản từ DB mang tiền tố `[LƯU]`. `invalidateQueries(audit)` **đã có sẵn** ở cả `activate` lẫn `releaseDispatch`. Narration LLM: `reply.content` ở lượt có tool_calls không còn bị vứt (§2.2), phát `source="llm"` và **không** chạy `_numbers_are_grounded` — có chủ ý, §4 Chặng 5 |
| MA-6.10 | `awaiting_approval` + dòng chờ treo ở đáy log | ✅ | MA-6.8 | `awaiting_approval` phát trước `run_finished`, **chỉ ở đường thành công**. Ba nhánh không phát: quality gate chặn · `not_required` · `decision` rỗng — hứa một cổng không mở là để người vận hành ngồi đợi vô ích. Client: `liveAwaitingSeq` tắt dòng treo khi duyệt **hoặc** từ chối; `revise` KHÔNG tắt vì v2 vẫn phải duyệt |
| MA-6.11 | Thẻ phương án mở đúng cửa duyệt cũ | ✅ | MA-6.10 | **Không cần code.** Thẻ QUICK APPROVE trong mockup chưa từng được dựng nên không có cửa thứ hai để tháo; nút của bảng chỉ huy vẫn gọi `openDialog("approve")`. Kiểm ra `approve.mutate` có hai chỗ gọi (console + `PlanDetail.tsx`), cả hai đều truyền `expectedVersion` nên đều hợp lệ — hệ quả: duyệt từ màn chi tiết không vào nhật ký console, đúng thiết kế (§9 #5) |
| MA-6.12 | `AGENT_OBSERVER` + allowlist chỉ-đọc + `POST /observe` | ✅ | MA-6.5 | `AGENT_OBSERVER` + `OBSERVER_TOOLS` + `POST/GET/DELETE /observe`, proxy NestJS `/operator/ai/observe`. Allowlist bằng đúng Assessment nhưng **nhỏ hơn hẳn registry**: `compute_relocation` vắng mặt vì chat đẻ ra phương án là đẻ ngoài cổng §11.1 |
| MA-6.13 | Ô nhập chế độ lệnh (`/forecast`, `/weather`, `/supply`) | ✅ | MA-6.12 | **Đảo so với kế hoạch:** LLM là đường chính, `intent.py` là đường đỡ khi LLM hỏng. Từ chối lệnh chạm cổng kiểm **trước** LLM; bảng từ khoá xếp nhóm cổng trước mọi nhóm khác, có test riêng cho "duyệt phương án" |
| MA-6.14 | Test: có phiên observer hay không, `decision` giống hệt từng byte | ✅ | MA-6.13 | `test_phien_hoi_dap_khong_lam_dich_mot_con_so_nao_cua_quyet_dinh`. Cùng khuôn MA-6.6 — điều luật mới kiểm được bằng máy, không chỉ ghi trong tài liệu |
| MA-6.15 | Chat chế độ LLM tự nhiên | ✅ | MA-6.14 | Gộp vào MA-6.13 vì LLM thành đường chính chứ không còn là phần tô thêm. Chạy thật trên mock: hỏi thời tiết → có số; "duyệt luôn đi" → bị từ chối; "chạy phân tích" → 23 → 30 dòng |
...
`situation_assessment` — không thể sinh ra dòng `[FORECAST_AGENT] > gọi get_weather()`;
(2) `astream_events` là async trong khi `run_pipeline` cố ý chạy sync trong `to_thread` vì MILP là
CPU-bound; (3) callback cho ra sự kiện mức node *và* mức tool *và* narration LLM từ một đường —
`.stream()` chỉ là tập con.
...
| MA-Q8 | ~~Đóng panel có được phép mất lượt phân tích đang chạy không?~~ **ĐÃ TRẢ LỜI: không.** | Popup nhật ký (MA-6.7) chết khi thu gọn thì vô nghĩa → nâng `usePipelineRun` lên `OperatorConsoleDashboard`, panel nhận `run` qua props. **Chấp nhận đánh đổi:** polling 2 giây chạy nền khi panel đóng mà run còn RUNNING (đã tự dừng ở DONE/FAILED) |
