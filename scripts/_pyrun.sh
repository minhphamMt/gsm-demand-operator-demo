...
# Tries python3 â†’ python â†’ py -3 on PATH; on Windows, falls back to common
...
#
# Exits 0 silently if no Python is found â€” hooks must never block the AI tool.
set -u
...
if command -v python3 >/dev/null 2>&1; then
  ...
  # PATH lookup failed â€” probe standard Windows install locations.
  PY=""
