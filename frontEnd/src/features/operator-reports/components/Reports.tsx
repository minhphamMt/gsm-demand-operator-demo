...
import { campaignsQuery, operationsReportQuery } from '@/features/operator-data'
...
export function Reports() {
  ...
  if (report.isPending || campaigns.isPending) return <Skeleton className="h-96" />
  const retryAll = () => { void report.refetch(); void campaigns.refetch() }
  if ((report.isError && report.data === undefined) || (campaigns.isError && campaigns.data === undefined)) return <ErrorState onRetry={retryAll} />
  ...
  return <div className="space-y-5">
    ...
    {report.data.campaigns.length ? <>
      <Card><div className="mb-3"><h2 className="font-semibold text-slate-950">Kết quả ghi trong DB theo campaign</h2><p className="mt-1 text-sm text-slate-500">Chỉ hiển thị tài xế kích hoạt và chuyến hoàn tất có bản ghi nguồn.</p></div><ReportCampaignChart campaigns={report.data.campaigns} /></Card>
      <CampaignReportTable campaigns={report.data.campaigns} />
    </> : <EmptyState description="Không có campaign nào trong bộ lọc phía server." title="Không có dữ liệu báo cáo" />}
    <ReportProvenance report={report.data} />
  </div>
}
