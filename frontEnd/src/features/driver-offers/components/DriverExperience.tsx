...
const zoneLabels: Record<string, string> = { 'zone-04': 'Hoàn Kiếm', 'zone-07': 'Tây Hồ', 'zone-10': 'Hoàng Mai' }
...
export function DriverExperience({ driverId = 'DRV-001', onSignOut }: { driverId?: string; onSignOut?: () => void }) {
  ...
  return <main className="mx-auto min-h-screen max-w-md bg-slate-50 p-4 text-slate-950">
    ...
    <section className="mt-5"><h2 className="mb-3 font-semibold">Lộ trình đang thực hiện</h2>{view.data.acceptedOffers.length ? view.data.acceptedOffers.map((offer) => <Card key={offer.id} className="mb-3"><div className="flex justify-between gap-3"><span className="font-medium">{zoneLabels[offer.targetZoneId] ?? offer.targetZoneId}</span><StatusBadge status="En route" /></div><p className="mt-2 flex items-center gap-1 text-sm text-slate-600"><Navigation className="size-4" />ETA {offer.etaMinutes} phút · thưởng {formatCurrency(offer.incentiveAmount)}</p></Card>) : <p className="text-sm text-slate-500">Chưa có lộ trình nào đang thực hiện.</p>}</section>
    <section className="mt-5"><div className="mb-3 flex items-center justify-between"><h2 className="font-semibold">Lịch sử của tôi</h2><span className="text-sm font-semibold text-brand-700">Tổng thưởng {formatCurrency(driver.rewardTotal)}</span></div><div className="space-y-2">{view.data.history.map((offer) => <Card key={offer.id} className="p-3"><div className="flex justify-between gap-3"><span className="text-sm font-medium">{zoneLabels[offer.targetZoneId] ?? offer.targetZoneId}</span><StatusBadge status={offer.status} /></div><p className="mt-1 flex items-center gap-1 text-xs text-slate-500"><Gift className="size-3" />{formatCurrency(offer.incentiveAmount)}</p></Card>)}</div></section>
  </main>
}
