...
export function OperatorMap({ forecastMinutes, onZoneSelect, selectedZoneId, zones }: OperatorMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addLayer({ id: 'zone-fill', type: 'fill', slot: 'middle', source: 'operator-zones', paint: { 'fill-color': ['case', ['==', ['get', 'dataStatus'], 'no_live_cells'], '#cbd5e1', ['match', ['get', 'severity'], 'Low', severityColors.Low, 'Medium', severityColors.Medium, 'High', severityColors.High, severityColors.Critical]], 'fill-opacity': ['case', ['boolean', ['feature-state', 'selected'], false], 0.88, ['==', ['get', 'dataStatus'], 'no_live_cells'], 0.38, 0.72], 'fill-emissive-strength': 1 } })
