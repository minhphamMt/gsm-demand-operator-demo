...
export function ZoneDetailsDialog({ forecastMinutes, onClose, zone }: ZoneDetailsDialogProps) {
  ...
  return (
    <Dialog isOpen={Boolean(zone)} onClose={onClose} title={zone ? `Chi tiết ${zone.label}` : 'Chi tiết vùng'}>
      {zone && <div className="space-y-4">
        <div className="overflow-hidden rounded-2xl bg-gradient-to-br from-brand-500 to-accent-500 p-4 text-white">
          ...
          <p className="mt-3 text-xs text-white/75">Ô H3 {zone.h3Index.slice(0, 10)}…</p>
        </div>
        ...
        <div className="rounded-xl border border-brand-100 bg-brand-50 p-3.5 text-sm text-brand-800"><p className="flex items-center gap-2 font-bold"><Sparkles className="size-4" />Khuyến nghị từ Agent</p><p className="mt-1.5 leading-5">Ưu tiên điều chuyển xe từ vùng lân cận; chỉ kích hoạt offer nếu mức thiếu vẫn còn cao sau ETA dự kiến.</p></div>
      </div>}
    </Dialog>
  )
}
