...
**Phiên bản:** 1.2 — Hợp nhất từ 3 tài liệu nguồn + **UI tài xế (Driver App)** (2026-08-04) + **đặc tả Baseline & Đối chứng** (2026-08-05)
...
| `enroute_supply` | int | Số xe đang di chuyển đến zone (do plan trước) |
...
```json
{
  ...
  "horizon_min": 15,
  "hotspots": [
    {"zone_id": 7, "is_hotspot": true, "gap": 41.0, "severity_score": 0.75}
  ],
  "surplus_zones": [
    {"zone_id": 12, "surplus": 15.0}
  ]
}
```
...
> **Ràng buộc nhất quán:** tổng số tài xế `status ∈ {online_idle, online_busy}` theo zone phải khớp `idle_supply` trong snapshot A1 tại cùng `ts_bucket`. Đây là điểm dễ lệch nhất giữa generator và driver registry — xem việc cần làm ở [Data-Contract-Data-AI.md](Data-Contract-Data-AI.md) (bộ A6).
...
3. Cộng 1 unit vào `enroute_supply` của `target_zone`, khả dụng sau `eta_steps` (dùng đúng `avg_vehicle_speed_kmh`).
4. Trừ 1 unit khỏi `idle_supply` zone nguồn nếu tài xế đang `online_idle`; nếu `offline` thì **không trừ ở đâu cả** — đây là cung mới thêm vào hệ thống.
...
| `supply_count[t-6..t0]`, `demand_count[t-6..t0]` | **Lookback N = 6 bước (30 phút)** — quyết định đã chốt #1 |
| Rolling mean/std trên window 30 phút | |
| `peak_flag`, `holiday_flag` | |
| `rain_mm_h` + lag trong window | |
| `rain_forecast_15/30` | input ngoại sinh |
| **`rain × peak` (tương tác)** | **Bắt buộc** — feature quyết định cho đúng problem statement |
...
- Báo cáo ablation: có/không nhóm feature mưa và `rain × peak`.
- Bảng metric tách 4 regime.
- `demand_p10`/`demand_p90` **không được `null`** trong output cuối cùng; kiểm tra calibration cơ bản (coverage p10–p90 xấp xỉ 80% trên test set).
...
với mỗi hotspot:
    tìm surplus zone gần nhất trong max_distance (ưu tiên eta ≤ 3 step)
    units = min(gap, surplus khả dụng, max_supply_move_pct × idle_supply nguồn)
    ràng buộc: zone nguồn không xuống dưới min_supply_per_zone
               zone nguồn không nằm trong cooldown_minutes
...
- **Áp phản hồi tài xế (v1.1):** mỗi `accept` cộng 1 unit vào `enroute_supply` zone đích sau `eta_steps` theo đúng quy tắc 4.9. Tài xế từ `offline` là **cung mới** (tổng cung tăng); tài xế từ `online_idle` là **dịch chuyển cung** (tổng cung không đổi) — Simulator phải phân biệt hai trường hợp, nếu không sẽ **thổi phồng** hiệu quả của activation.
